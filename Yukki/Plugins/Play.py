...
async def Music_Stream(_, CallbackQuery):
    ...
    mystic = await CallbackQuery.message.reply_text(
        f"**{MUSIC_BOT_NAME} Trình tải xuống**\n\n**Title:** {title[:50]}\n\n0% ▓▓▓▓▓▓▓▓▓▓▓▓ 100%"
    )
...
async def slider_query_results(_, CallbackQuery):
    ...
    if what == "B":
        if type == 0:
            ...
            query_type = int(type - 1)
        await CallbackQuery.answer("Nhận kết quả trước", show_alert=True)
        (
            title,
            duration_min,
            duration_sec,
